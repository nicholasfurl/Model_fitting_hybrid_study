...
%v4 accommodates new versions of biased prior (PM) biased variance (PV)
%and 
...
%1 cutoff, 2 Cs, 3 IO, (beta only), 4 BV, 5 BR, 6 BPM, 7 Opt, 8 BPV, 9 BOV
...
do_models = [7 9 6 8];    %These are now what v2 called model identifiers - Applies at the moment to both make_model_data and check_params;

%File I/O
if use_file ~=1;
    comment = sprintf('out_sahira_BPMBPVtest_pay%dvals%dstudy%d',payoff_scheme,subjective_vals,study);     %The filename will already fill in basic parameters so only use special info for this.
end;
...
model_names = {'CO' 'CS' 'IO' 'BV' 'BR' 'BPM' 'OM' 'BPV' 'BOV'}; %IO is a placeholder, don't implement

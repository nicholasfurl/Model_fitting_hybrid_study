...
do_models = [4 ];    %These are now what v2 called model identifiers - Applies at the moment to both make_model_data and check_params;
IC = 2; %1 if AIC, 2 if BIC
log_or_not = 0; %obsolete

%File I/O
if use_file_for_plots ~=1;
    comment = sprintf('out_sahira_testBV_ll%dpay%dvals%dstudy%d',all_draws_set,payoff_scheme,subjective_vals,study);     %The filename will already fill in basic parameters so only use special info for this.
    %          comment = 'sahiraLimits';
    %          comment =sprintf('out_NEWlimits_ll%dpay%dvals%d',all_draws_set,payoff_scheme,subjective_vals);
end;
...
%filename_for_plots = [outpath filesep 'out_sahira_fixranks_ll1pay3vals1study520232308.mat']; %Unfortunately still needs to be typed in manually
...
%(I would update mean and variance directly from Generate_params).identifier ==

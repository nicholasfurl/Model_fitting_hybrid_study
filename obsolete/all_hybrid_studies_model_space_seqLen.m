...
%desiognated in the sequence length condition column but phase 2 rows are,
...
%continue t be the one for NEW. Another one will be
...
% comment = sprintf('out_test_jscriptSeqLen_Pay%dvals%dLen%d',payoff_scheme,subjective_vals,seq_cond);     %The filename will already fill in basic parameters so only use special info for this.
    comment =sprintf('out_SeqLen_limits_ll%dpay%dvals%dseqcond%d',all_draws_set,payoff_scheme,subjective_vals,seq_cond);
%     comment = 'test';
% end;
outpath = 'C:\matlab_files\fiance\parameter_recovery\beta_fixed_code\Model_fitting_hybrid_study\outputs';
% filename_for_plots = [outpath filesep 'out_new_ll1_out_test_jscriptSeqLen_Pay1vals1_20231306.mat']; %Unfortunately still needs to be typed in manually

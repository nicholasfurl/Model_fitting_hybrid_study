...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = hybrid_parameter_recovery_v1;

%13th Sept 2024: modified the version of imageTask_paper_parameter_recovery_v5.m that I
%found here. Modified it to better work with hybrid and renamed
%hybrid_parameter_recovery_v1.m to distiniguish a new branch for the comms
%psych submission (now in second review round).
...
%1 cutoff, 2 Cs, 3 dummy (vestige), 4 BV, 5 BR, 6 BPM, 7 Opt, 8 BPV
do_models = [2 1 6 4 5];    %These are now what v2 called model identifiers - Applies at the moment to both make_model_data and check_params;
...
%These correspond to identiiers (not configured implementations like in v2) in the v3_sweep version
...
if make_config_model_data == 1;
    ...
    analysis_name = sprintf(...
        'out_PR_%dmodels%dmodelTypes%dparamLevels%dbetaLevels%dsubs%dseqs%dopts_%s_'...
...
if check_params == 1;
    ...
    for model = do_models;
        ...
        for sub = 1:Generate_params.num_subs;
            ...
            disp(...
                sprintf('fitting modeli %d name %s param %3.2f subject %d' ...
                , model ...
...
cmap = [
    ...
    1.0000, 0, 0;    % Red
    0, 0, 1.0000;    % Blue
    0.6667, 0, 1.0000; % Violet
    1 1 1 ;              %placeholder#
    1 1 1               %placeholder
    ];

cmap = hsv(8+1);  %models + subjects

plot_cmap = cmap;

% new_order = [7, 5, 6, 8, 1, 2, 4];
% plot_cmap = cmap(new_order,:)

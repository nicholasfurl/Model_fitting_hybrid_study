...
if get_data == 1;
    ...
    save('data_struct2.mat','data'); %takes a long time to run and I might want to skip to here and load data later
    
    
end;    %get data?

load('data_struct.mat','data'); %takes a long time to run and I might want to skip to here and load data later
...
for study = 1:data.num_studies;
    ...
    %participants
    bar( x_axis_it, mean(mean(data.study(study).samples)), 'FaceAlpha',.1, 'FaceColor', plot_colors(x_axis_it,:));
    plotSpread( mean(data.study(study).samples)' , 'xValues', x_axis_it, 'distributionColors',plot_colors(x_axis_it,:) );
    ...
    for model = 1:num_models;
        ...
        bar( x_axis_it, mean(mean(data.study(study).model(model).samples)), 'FaceAlpha',.1, 'FaceColor', plot_colors(x_axis_it,:));
        plotSpread( mean(data.study(study).model(model).samples)' , 'xValues', x_axis_it, 'distributionColors',plot_colors(x_axis_it,:) );
